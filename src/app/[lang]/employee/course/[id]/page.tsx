...
export default function CoursePage({ params }: { params: { lang: string; id: string } }) {
  ...
  // Mock course data
  const course = {
    title: params.id === "1" ? "Secure API Coding" : 
           params.id === "2" ? "Social Engineering" : 
           params.id === "3" ? "Phishing Awareness" : "Enterprise Security Basics",
    description: "Learn the fundamentals of keeping your enterprise safe.",
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      <header className="border-b border-border px-6 py-4 flex items-center justify-between sticky top-0 z-10 bg-background/95 backdrop-blur">
        <div className="flex items-center gap-4">
          <Link href={`/${params.lang}/employee/dashboard`} className="text-muted-foreground hover:text-foreground transition-colors">
